...
object ResponseCollector {
  def apply[T](
    ...
    val ref = factory.actorOf( props( label, tracker, result, reducer )( matcher ), s"${label}-collector-${ShortUUID()}" )
    ( result.future, ref )
  }
  ...
  def props[T](
    ...
    Props(
      new ResponseCollector[T]( label, timeout.duration, initialTracker, result, matcher, reducer ) with ConfigurationProvider {
  ...
  object Countdown {
    ...
    final case class SimpleCountdown[T] private[ResponseCollector]( expectedMessages: Int ) extends Tracker[T] {
      override def addResponse( response: T ): Tracker[T] = copy( expectedMessages = (expectedMessages - 1) max 0 )
  ...
  object MatchIds {
    ...
    final case class SimpleMatchIds[M, I] private[ResponseCollector](
      ...
      override def addResponse( response: M ): Tracker[M] = copy( expectedIds = expectedIds -- toIds(response) )
...
class ResponseCollector[T](
  ...
  def scheduleWhistle( untilWhistle: FiniteDuration = outer.warningBudget(timeout) ): Unit = {
    cancelWhistle()
    pendingWhistle = Some( context.system.scheduler.scheduleOnce(untilWhistle, self, ReceiveTimeout) )
  }
  ...
  override def receive: Receive = LoggingReceive { around( ready(Vector.empty[T], initialTracker) ) }

  private def ready( responses: Vector[T], tracker: Tracker[T], warningsRemaining: Int = outer.warningsBeforeTimeout ): Receive = {
    case m if matcher isDefinedAt m => {
      ...
      else context become LoggingReceive { around( ready(nextResponses, nextTracker, warningsRemaining) ) }
    }
