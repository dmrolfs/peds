...
class AgentProjection[T](
  ...
  private val log = Logger( "AgentProjection" )

  def start(): Future[T] = materializeCurrentView map {
    case ( current, snr ) =>
      log.warn( s"#TEST:DEBUG: current materialization: [${current}]" )
      log.info( "current view materialized. starting ongoing projection..." )
  ...
  private def materializeCurrentView: Future[( T, Long )] = {
    log.info( "starting active projection source..." )
  ...
  private def startProjectionFrom( sequenceId: Long ): Unit = {
    log.info( "starting active projection" )
    queryJournal
      .eventsByTag( tag, Offset.sequence( sequenceId ) )
      .map { e =>
        log.warn( s"TEST AgentProjection processing new tagged event:[${e}]" ); e
      }
      .via( filterKnownEventsFlow )
      .to( sink )
      .run()
  ...
  private def filterKnownEventsFlow: Flow[EventEnvelope, Directive[T], NotUsed] = {
    Flow[EventEnvelope]
      .map { e =>
        log.warn( s"TEST enter filter - tagged event:[${e}]" ); e
      }
      .collect {
  ...
  private def sink: Sink[Directive[T], Future[( T, Long )]] = {
    Sink.foldAsync( ( view.get(), 0L ) ) {
      case ( ( _, lastSnr ), d ) =>
        log.warn( s"#TEST applying directive:[${d}] @ snr:[${d.sequenceNr}]" )
        view
          .alter { d.lens }
