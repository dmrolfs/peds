...
object AskRetry {
  ...
  implicit class RetryingActorRef( val underlying: ActorRef ) extends AnyVal {
    ...
    /**
      ...
      */
    def askretry[T]( msg: T, maxAttempts: Int, rate: FiniteDuration )(
  ...
  /**
    ...
    */
  case class RetryException( attempts: Int )
      extends Exception( s"Cannot retry after ${attempts} attempts" )

  /**
    * Case class for the message that is used to start the RetryingActor routine
    */
  private[akka] case class Ask[T](
    target: ActorRef,
    message: T,
    rate: FiniteDuration,
    maxAttempts: Int
