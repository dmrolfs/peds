...
case class TelecomAddress(
  ...
  override def address: String = {
    ...
    val countryCode = locale2CountryCode( locale ).map { cd =>
      if (cd.isEmpty) cd else (cd + " ")
    }
...
object TelecomAddress {
  ...
  def locale2CountryCode( isoCountry: String ): Option[String] =
    localePhoneCountryCodes get isoCountry
