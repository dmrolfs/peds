...
trait Effectivity {
  ...
  def effectiveOn( date: => LocalDate = { LocalDate.now } ): Boolean =
    effective( date.toDateTimeAtStartOfDay )
  ...
  def isExpired( datetime: => DateTime = { DateTime.now } ): Boolean =
    validTo map { _ <= datetime } getOrElse false
}
...
object EffectiveRange {
  ...
  def apply( validFrom: DateTime, validTo: DateTime ): EffectiveRange =
    new EffectiveRange( validFrom, validTo )
...
case class Enablement(
  ...
  override def toString: String =
    s"""(${if (enabled) "enabled" else "disabled"} for [${validFrom getOrElse ""}, ${validTo getOrElse ""}))"""
}
