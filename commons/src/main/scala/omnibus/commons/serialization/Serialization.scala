...
class Serialization( config: Config ) {
  ...
  /**
    ...
    */
  def deserialize[T]( bytes: Array[Byte], serializerId: TypeIdentifier, clazz: Option[Class[_ <: T]] ): Try[T] = {
    Try {
      val serializer = try {
  ...
  /**
    ...
    * Returns either the resulting object or an Exception if one was thrown.
    */
  def deserialize( bytes: Array[Byte], serializerId: TypeIdentifier, manifest: String ): Try[AnyRef] = {
    Try {
      val serializer = try {
        getSerializerById( serializerId )
      } catch {
