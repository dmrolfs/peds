...
trait XmlDeserializer[T] extends Deserializer[Node, T] {
  ...
  def deserialize( node: Node )( implicit c: ConvertFrom[Node, T] ): Either[Throwable, T] = {
    try { Right( c( node ) ) } catch { case NonFatal( e ) => Left( e ) }
  }
}
...
object XmlDeserializer {
  ...
  def fromXml[T]( n: Node )( implicit c: ConvertFrom[Node, T] ) =
    new XmlDeserializer[T] {}.deserialize( n )

  def forTraversable[T]( src: Node )(
    implicit c: ConvertFrom[Node, T]
  ): Either[Throwable, Traversable[T]] = trace.block( "forTraversable" ) {
    try {
      Right(
        src.child map { e =>
          trace( "child=" + e )
  ...
  def forOption[T]( parent: Node, target: String )(
    implicit c: ConvertFrom[Node, T]
