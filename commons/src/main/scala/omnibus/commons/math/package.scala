...
package object math {
  ...
  object Interpolator {
    ...
    def apply( xs: Array[Double], ys: Array[Double] ): AllIssuesOr[Interpolator] = {
      checkDimensions( xs, ys ) map {
        case ( x, y ) => if (x.size < 3) linearInterpolator( x, y ) else splineInterpolator( x, y )
      }
    }
    ...
    def checkDimensions(
      xs: Array[Double],
      ys: Array[Double]
    ): AllIssuesOr[( Array[Double], Array[Double] )] = {
    ...
    final case class NumberIsTooSmallError private[math] ( xs: Array[Double] )
        extends IllegalArgumentException(
          s"""not enough xs [${xs.mkString( "," )}] to interpolate"""
        )
        with MathError

    final case class MismatchedDimensionsError private[math] (
      xs: Array[Double],
      ys: Array[Double]
    ) extends IllegalArgumentException(
          s"""xs [${xs.mkString( "," )}] size different than ys [${ys.mkString( "," )}]"""
        )
        with MathError
  }
