...
object StringWithSimilarity {
	val trace = peds.commons.log.Trace[StringWithSimilarity]
	...
	def determineMatchesAndTransposes( lhs: String, rhs: String ): MatchResults = {
		...
		for ( i <- 0 until shortest.length ) {
			if( shortest(i) == longest(i) ) numberOfMatches += 1
				...
				//given the string's size and the window size
				val forwardBoundary = if ( (i + window) > (longest.length - 1) ) ( longest.length - 1 ) else ( i + window )
	 ...
	 */
	def jaroDistance( numMatches: Int, numTransposes: Int, lhsLength: Int, rhsLength: Int ): Double = {
