...
class PartialParserSpec extends FlatSpec with Matchers {
  ...
  it should "produce a single prime criterion with property" in {
    parser.parse( "foo+sort=bar" ) shouldBe CompositeCriterion(
      Map(),
      "foo" -> PrimeCriterion( Map( "sort-asc" -> "bar" ) )
    )
  }

  it should "produce a composite criterion with single spec" in {
    parser.parse( "foo:(bar)" ) shouldBe CompositeCriterion(
      Map(),
      "foo" -> CompositeCriterion( Map(), "bar" -> PrimeCriterion() )
    )
    parser.parse( "(foo:(bar))" ) shouldBe CompositeCriterion(
      Map(),
      "foo" -> CompositeCriterion( Map(), "bar" -> PrimeCriterion() )
  ...
  it should "produce a composite criterion with multiple subcriteria" in {
    parser.parse( "foo:(bar, zed)" ) shouldBe CompositeCriterion(
      Map(),
      "foo" -> CompositeCriterion( Map(), "bar" -> PrimeCriterion(), "zed" -> PrimeCriterion() )
    )
    parser.parse( "foo:bar, zed" ) shouldBe CompositeCriterion(
      Map(),
      "foo" -> CompositeCriterion( Map(), "bar" -> PrimeCriterion(), "zed" -> PrimeCriterion() )
    )
    parser.parse( "foo:bar, zed, ted:ma,pa" ) shouldBe CompositeCriterion(
      Map(),
      "foo" -> CompositeCriterion(
        Map(),
        "bar" -> PrimeCriterion(),
        "zed" -> PrimeCriterion(),
        "ted" -> CompositeCriterion( Map(), "ma" -> PrimeCriterion(), "pa" -> PrimeCriterion() )
      )
    )
  }
  ...
  it should "product deep nested composite" in {
    parser.parse(
      "BGC:product:USOneSearch:response:detail:offenders:offender:identity:personal:fullName"
    ) shouldBe CompositeCriterion(
      Map(),
      "BGC" -> CompositeCriterion(
        Map(),
        "product" -> CompositeCriterion(
          Map(),
          "USOneSearch" -> CompositeCriterion(
            Map(),
            "response" -> CompositeCriterion(
              Map(),
              "detail" -> CompositeCriterion(
                Map(),
                "offenders" -> CompositeCriterion(
                  Map(),
                  "offender" -> CompositeCriterion(
                    Map(),
                    "identity" -> CompositeCriterion(
                      Map(),
                      "personal" -> CompositeCriterion( Map(), "fullName" -> PrimeCriterion() )
                    )
                  )
                )
              )
  ...
  it should "produce complex mixed criteria" in {
    ...
    parser.parse( "augur, foo:(bar, zed, ted:ma,pa), seahawks" ) shouldBe CompositeCriterion(
      Map(),
    ...
    parser.parse( "foo:(bar, zed, ted:ma,pa), augur, seahawks" ) shouldBe CompositeCriterion(
      Map(),
      "augur" -> PrimeCriterion(),
      "foo" -> CompositeCriterion(
        Map(),
  ...
  it should "produce mix of prime and multiple composite" in {
    parser.parse(
      ...
      "report" -> CompositeCriterion(
        Map(),
        "offender" -> CompositeCriterion(
          Map(),
          "address" -> CompositeCriterion( Map(), "addressLine" -> PrimeCriterion() )
        )
      )
    )
    ...
    parser.parse(
      ...
      "report" -> CompositeCriterion(
        ...
        "offender" -> CompositeCriterion(
          Map(),
          "offensesByDegree" -> CompositeCriterion(
            Map(),
            "Other" -> CompositeCriterion(
              Map(),
