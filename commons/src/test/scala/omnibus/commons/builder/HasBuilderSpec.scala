 ...
 *
 *   This file was part of "shapeless-builder". Incorporated directly here to establish
 *   original contract. If this test fails, then it's due to underlying library change
 *   in dmrolfs/shapeless-builder fork.
 *
...
package omnibus.commons.builder
...
class HasBuilderSpec extends FlatSpec with Matchers {
  ...
  "A builder for a case class with a complex optional parameter" should "generate the expected case class" in {
    ...
    // Set to `Some(...)`
    builder.set( X, Some( "Hello" ) ).build() should equal(
      TestOptionStringOptional( Some( "Hello" ) )
    )
  }
